...
7. `commit(cwd, message)`, then `push(cwd)` if `commitflow-ai.autoPush` is
   `true` (default). `commit(cwd, message)` strips any null bytes from
   `message` before invoking git — `execFile` throws
   (`ERR_INVALID_ARG_VALUE`) on an arg containing `\x00`, and some models
   occasionally emit stray control characters. `push(cwd)` runs plain
   `git push`; if that fails because the current branch has no upstream
   yet, it retries with `git push --set-upstream origin <branch>` instead
   of surfacing an error for what is really a first-push-on-a-new-branch
   situation.
